...
function friendlyError(msg: string): string {
  ...
  if (lower.includes("already registered") || lower.includes("already been registered")) return "Account exists. Sign in instead.";
...
export default function SignupPage() {
  ...
  const handleOAuth = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? friendlyError(err.message) : "Google sign-up failed";
      setError(msg);
      toast.error(msg);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? friendlyError(err.message) : "Signup failed";
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4">
      <div className="w-full max-w-md space-y-6">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">Three AI surfaces, one workspace</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Input id="name" type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Ada Lovelace" autoComplete="name" disabled={isLoading} autoFocus />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email" disabled={isLoading} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="password">Password (min 8 chars)</Label>
              <div className="relative">
                <Input id="password" type={showPassword ? "text" : "password"} required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" autoComplete="new-password" disabled={isLoading} className="pr-10" />
